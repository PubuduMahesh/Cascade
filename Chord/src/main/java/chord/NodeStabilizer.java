        ...
        try {
            ...
            while (true) {
                ...
                if (!this.chordNode.getAddress().equals(this.chordNode.getFirstSuccessor().getAddress()) || (this.chordNode.getPort() != this.chordNode.getFirstSuccessor().getPort())) {
                    ...
                    String message = ChordOperation.REQUEST_PREDECESSOR + " " + this.chordNode.getId() + " asking " + this.chordNode.getFirstSuccessor().getId();
                    ...
                    if (!this.chordNode.getAddress().equals(predecessorAddress) || (this.chordNode.getPort() != predecessorPort)) {
                        ...
                        String message1 = ChordOperation.JOIN + " " + this.chordNode.getAddress() + " " + this.chordNode.getPort();
                    ...
                    for (int i = 0; i < 32; i++) {
                        ...
                        // Send query to chord
                        String message1 = ChordOperation.FIND_NODE + " " + bigResult.longValue();
                        message1 = Message.customFormat("0000", message1.length()) + " " + message1;
                        byte[] toSend1  = message1.getBytes(); 
                        InetAddress IPAddress1; 
                        try {
                    ...
                    for (int i = 0; i < 32; i++) {
                        ...
                        // Send query to chord
                        String message = ChordOperation.FIND_NODE + " " + bigResult.longValue();
                        message = Message.customFormat("0000", message.length()) + " " + message;
                        byte[] toSend  = message.getBytes(); 
                        InetAddress IPAddress; 
                        try {
                ...
                for(int i = 0; i < chordNode.getKeys().size(); i++)
                {
                    ...
                    String message = ChordOperation.FIND_NODE + " " + Long.parseLong((String) chordNode.getKeys().keySet().toArray()[i]);
                    message = Message.customFormat("0000", message.length()) + " " + message;
                    byte[] toSend  = message.getBytes(); 
                    InetAddress IPAddress; 
                    try {
                    ...
                    if(!this.chordNode.getAddress().equals(addressFragments[0]) || (this.chordNode.getPort() != Integer.valueOf(addressFragments[1])))
                    {
                        ...
                        chordNode.getKey(id);
                        message = ChordOperation.STORE + " " + Long.parseLong(id) + " " + chordNode.getKey(id).size();
