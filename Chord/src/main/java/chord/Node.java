...
import chord.operation.DownloadListener;
            ...
            try {
                ...
                for (int i = 0; i < 32; i++) {
                    ...
                    try {
                        ...
                        System.out.println("sending message:" + message + "\nfrom-" + HostConfiguration.MY_IP + ":" +
                                           HostConfiguration.MY_PORT + ",to-" + this.existingNodeAddress + ":" + this.existingNodePort);
                        try {
                            socket.send(packet);
    ...
    private void initializeSuccessors() {
        ...
        if (!this.address.equals(this.firstSuccessor.getAddress()) || (this.port != this.firstSuccessor.getPort())) {
            ...
            try {
                ...
                String message = ChordOperation.JOIN + " " + this.getAddress() + " " + this.getPort();
                message = Message.customFormat("0000", message.length()) + " " + message;
                byte[] toSend  = message.getBytes(); 
                InetAddress IPAddress; 
                try {
                    ...
                    System.out.println("sending message:" + message + "\nfrom-" + HostConfiguration.MY_IP + ":" +
                                       HostConfiguration.MY_PORT + ",to-" + this.firstSuccessor.getAddress() + ":" + this.firstSuccessor.getPort());
                    try {
                        socket.send(packet);
                    } catch (IOException ex) {
                        Logger.getLogger(Node.class.getName()).log(Level.SEVERE, null, ex);
